     ...
     (use-package ob-tangle
       ...
       (progn
         ...
         byte-compile the result."
           (progn
             (org-babel-tangle-file init-file init-file-el)
        ...
        (if osx?
            (osascript-run-str
             (concat
              ...
              " && exit\""))
          (apply #'call-process "xterm" nil 0 nil "-e" (list "/bin/zsh" "-c" (mapconcat 'identity args " ")))))
              ...
              (pdflatex
               (concat "-pdflatex='"
                       (if latexcmd (concat (second latexcmd) "latex")
                         "xelatex")
                       "'"))
              (tex-name  (concat (file-name-sans-extension buffer-file-name)
                                             ".tex")))
         (if (= arg 4)
