   ...
   #+BEGIN_SRC emacs-lisp
     (use-package smart-mode-line
       :config (progn
               ...
               (add-to-list 'sml/replacer-regexp-list '("^~/.julia/dev" ":JD:") t)
...
**** Auto-dark
     ...
     #+BEGIN_SRC emacs-lisp
       (defun list-buffers-with-mode (mode)
         (-filter
          (lambda (buf) (eq (buffer-local-value 'major-mode buf) mode))
          (buffer-list)))

       (defun revert-all-pdfs ()
         (mapcar
          (lambda (buf) (with-current-buffer buf (pdf-view-revert-buffer t t)))
          (list-buffers-with-mode 'pdf-view-mode)))

       (defun auto-dark-hook-fun ()
         (progn
           (set-fonts)
           (revert-all-pdfs)))
       ...
       (use-package auto-dark
         ...
         (auto-dark-mode 1)
         (add-hook 'auto-dark-dark-mode-hook 'auto-dark-hook-fun)
         (add-hook 'auto-dark-light-mode-hook 'auto-dark-hook-fun))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun reinstall-treesit-grammars ()
    (interactive)
    (mapc #'treesit-install-language-grammar '(bash c cpp elixir erlang fortran heex html julia make markdown toml yaml)))
#+END_SRC
** Corfu
#+BEGIN_SRC emacs-lisp
  (use-package corfu
...
** Erlang
#+BEGIN_SRC emacs-lisp
  ;; See also https://github.com/sebastiw/edts

  (setq erlang-root-dir (s-trim (shell-command-to-string "asdf where erlang")))
  (if (f-directory? erlang-root-dir)
      (progn
       (let ((erlang-emacs-dir (-first-item (f-glob (f-join erlang-root-dir "lib" "tools-*" "emacs")))))
         (if erlang-emacs-dir
             (setq load-path (-uniq (cons erlang-emacs-dir load-path)))
           (error (s-concat "Could not find Emacs tooling in Erlang install: " erlang-root-dir))))
       (setq exec-path (-uniq (cons (f-join erlang-root-dir "bin") exec-path)))))
  (use-package erlang-start
    :straight nil)
  (use-package erlang-ts-mode
    :straight (erlang-ts-mode :type git :host github :repo "wingyplus/erlang-ts-mode")
    :mode "\\.erl$")
#+END_SRC
